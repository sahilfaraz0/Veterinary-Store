...
function renderPosGrid() {
  ...
  container.innerHTML = filtered.map(p => {
    ...
    return `
      <div onclick="${isOut || isBlister ? '' : `addToCart('${p.id}', false)`}" class="p-4 rounded-2xl border ${isOut ? 'bg-slate-100 dark:bg-slate-800/30 border-slate-200 dark:border-slate-800 opacity-60 cursor-not-allowed' : 'bg-white dark:bg-slate-800/80 hover:bg-slate-50 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-700 cursor-pointer shadow-sm hover:shadow-md'} transition-all flex flex-col justify-between space-y-3">
        <div class="flex items-start justify-between gap-2">
          <div class="flex items-center gap-3 min-w-0">
            <div class="w-11 h-11 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 overflow-hidden flex-shrink-0 flex items-center justify-center">
              ${p.image_url ? `<img src="${p.image_url}" alt="" class="w-full h-full object-cover" />` : `<i data-lucide="pill" class="w-5 h-5 text-emerald-600"></i>`}
            </div>
            <div class="min-w-0">
              <h4 class="font-bold text-sm text-slate-900 dark:text-slate-100 truncate" title="${p.name}">${p.name}</h4>
              <p class="text-xs text-slate-500 dark:text-slate-400 font-mono">SKU: ${p.barcode} • GST: ${taxRate}%</p>
            </div>
          </div>
          ${p.requires_prescription ? `
            <span class="px-2 py-0.5 rounded bg-amber-100 dark:bg-amber-950/80 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800 text-[10px] font-bold font-mono flex-shrink-0">Rx</span>
          ` : `
            <span class="px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-950/80 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 text-[10px] font-bold font-mono flex-shrink-0">OTC</span>
          `}
        </div>

        ${isBlister ? `
          <div class="pt-2 border-t border-slate-100 dark:border-slate-700/60 font-mono text-xs flex flex-col gap-2">
            <div class="flex items-center justify-between">
              <span class="font-extrabold text-emerald-600 dark:text-emerald-400 text-sm">${formatCurrency(price)} / ${p.unit}</span>
              <span class="px-2 py-0.5 rounded font-bold text-[11px] ${isOut ? 'bg-red-100 dark:bg-red-950/80 text-red-700 dark:text-red-300' : 'bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300'}">
                ${isOut ? 'Out of Stock' : `${Math.round(totalQty * 100) / 100} packs (${Math.round(totalQty * packSize)} tabs)`}
              </span>
            </div>
            ${isOut ? '' : `
            <div class="grid grid-cols-2 gap-1.5 pt-0.5">
              <button onclick="event.stopPropagation(); addToCart('${p.id}', false)" class="py-1.5 px-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-sm transition-all text-center">
                + Pack (${formatCurrency(price)})
              </button>
              <button onclick="event.stopPropagation(); addToCart('${p.id}', true)" class="py-1.5 px-2 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs shadow-sm transition-all text-center" title="Sell single tablet/dose from blister pack">
        ...
        ` : `
          <div class="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-700/60 font-mono text-xs">
            <span class="font-extrabold text-emerald-600 dark:text-emerald-400 text-sm">${formatCurrency(price)}</span>
            <span class="px-2.5 py-1 rounded-lg font-bold ${isOut ? 'bg-red-100 dark:bg-red-950/80 text-red-700 dark:text-red-300' : 'bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300'}">
