  ...
  if (btnMarkAllChecked) {
    btnMarkAllChecked.addEventListener('click', () => {
      if (appState.notifications) {
        ...
        localStorage.setItem('vet_store_notifications', JSON.stringify(appState.notifications));
      }
      renderAdminAlerts();
    ...
    return `
      <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors border-b border-slate-100 dark:border-slate-800/60">
        ...
        <td class="py-4 px-6 text-xs font-semibold text-slate-700 dark:text-slate-300">
          ${t.cashier_name || 'Demo Cashier (Staff)'}
        </td>
...
function reprintReceipt(txnId) {
  ...
  openInvoiceModal(
    txn.id,
    txn.receipt_number,
    txn.cashier_name || 'Demo Cashier (Staff)',
...
function renderAdminAlerts() {
  const saved = localStorage.getItem('vet_store_notifications');
  if (saved) {
    try {
...
async function voidTransaction(transactionId) {
  const user = appState.currentUser || { full_name: 'Staff Member', role: 'Cashier' };
  ...
  const modifierName = user ? `${user.full_name || 'Staff'} (${user.role || 'Cashier'})` : 'Staff Member';
  const roleStr = user?.role || 'Cashier';
  ...
  appState.notifications.unshift({
    ...
    timestamp: new Date().toISOString(),
    cashier_name: modifierName,
    action: `VOIDED Sale ${txn.receipt_number} (${formatCurrency(txn.total_amount)}). Stock quantities restored.`,
    checked: false
  });
  localStorage.setItem('vet_store_notifications', JSON.stringify(appState.notifications));

  if (typeof logAuditEvent === 'function') {
    logAuditEvent('SALE_VOIDED', `${roleStr} (${user?.full_name || 'Staff'}) voided sale receipt ${txn.receipt_number} (${formatCurrency(txn.total_amount)}) and restored stock quantities.`, transactionId);
  }

  if (roleStr !== 'Admin') {
    showToast('Sale voided successfully! Stock restored. Admins have been notified of this void operation.', 'info');
  } else {
