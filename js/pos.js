...
function renderPosGrid() {
  ...
  appState.posLimit = appState.posLimit !== undefined ? appState.posLimit : 5;
  const visibleList = filtered.slice(0, appState.posLimit);
  ...
  if (filtered.length > appState.posLimit) {
    const remaining = filtered.length - appState.posLimit;
    const labelText = remaining > 10 ? `Next 10 of ${remaining} remaining` : `${remaining} remaining`;
    const btnDiv = document.createElement('div');
    btnDiv.className = 'pt-3 pb-2 flex justify-center';
    btnDiv.innerHTML = `
      <button onclick="loadMorePosMedicines()" class="px-6 py-3 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold text-xs flex items-center gap-2 shadow-sm transition-all border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500">
        <i data-lucide="chevron-down" class="w-4 h-4 text-emerald-500"></i>
        <span>See More (${labelText})</span>
      </button>
    `;
...
function loadMorePosMedicines() {
  appState.posLimit = (appState.posLimit !== undefined ? appState.posLimit : 5) + 10;
  renderPosGrid();
}
window.loadMorePosMedicines = loadMorePosMedicines;
...
function confirmRxVerification() {
  ...
  const docSelect = document.getElementById('rx-doctor-select');
  const firstDoc = appState.vetDoctors[0];
  const firstDocName = typeof firstDoc === 'object' && firstDoc ? firstDoc.name : firstDoc;
  const doctorName = docSelect && docSelect.value ? docSelect.value : (firstDocName || 'Verified Vet Doctor');
...
function renderPosCart() {
  ...
  const summaryEl = document.getElementById('pos-checkout-summary');
  if (summaryEl) {
    if (cart.length === 0) {
...
async function completeSale() {
  ...
  const cashier = appState.currentUser || { id: 'cashier-demo-id', full_name: 'Demo Cashier' };

  const petSelect = document.getElementById('pos-pet-link-select') || document.getElementById('pos-pet-select');
  const selectedVal = petSelect && petSelect.value ? petSelect.value : null;
  let customerName = 'Walk-in Client';
  let customerPhone = 'N/A';
  let petInfo = null;

  if (selectedVal) {
    if (selectedVal.startsWith('owner-')) {
      const oId = selectedVal.replace('owner-', '');
      const owner = (appState.owners || []).find(o => o.id === oId);
      if (owner) {
        customerName = owner.name;
        customerPhone = owner.phone;
        petInfo = { pet_id: null, pet_name: 'N/A', pet_species: 'N/A', owner_id: owner.id, owner_name: owner.name, owner_phone: owner.phone };
      }
    } else {
      const pet = (appState.pets || []).find(p => p.id === selectedVal);
      if (pet) {
        ...
        if (owner) {
          ...
          customerName = 'Unknown Owner';
          petInfo = { pet_id: pet.id, pet_name: pet.name, pet_species: pet.species, owner_id: null, owner_name: 'Unknown Owner', owner_phone: 'N/A' };
        }
      }
    }
  }
  ...
  if (appState.mode === 'SUPABASE' && window.supabaseClient) {
    try {
      ...
      const snapshotItems = appState.cart.map(item => ({
        product_id: item.product_id,
        name: item.name,
        quantity: item.quantity,
        unit_price: item.unit_price,
        tax_rate: item.tax_rate !== undefined ? item.tax_rate : 18
      }));
      ...
      openInvoiceModal(txnId, receiptNum, cashier.full_name, selectedPaymentMethod, hasRx, vetDoctor, snapshotItems, petInfo);
      appState.cart = [];
      renderPosCart();
      return;
    } catch (err) {
...
function openInvoiceModal(txnId, receiptNum, cashierName, paymentMethod, hasRx, vetDoctor, cartSnapshot = null, petInfo = null) {
  ...
  const subtotalDisplay = (txn && txn.subtotal_amount !== undefined && txn.subtotal_amount > 0) ? txn.subtotal_amount : totalSub;
  const taxDisplay = (txn && txn.total_tax !== undefined && (txn.total_tax > 0 || totalGST === 0)) ? txn.total_tax : totalGST;
